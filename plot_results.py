...
grouped = df.groupby(["encoder", "model"])[["acc_presence", "acc_salience"]].mean().reset_index()

# Melt into long format
melted = grouped.melt(id_vars=["encoder", "model"],
                      value_vars=["acc_presence", "acc_salience"],
                      var_name="metric", value_name="accuracy")

# One plot per metric (presence/salience), comparing encoders & models
for metric in ["acc_presence", "acc_salience"]:
